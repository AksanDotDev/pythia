...
class Teryte(commands.Cog):
    ...
    async def group_root(self, ctx):
        await ctx.send(
                "<https://io.aksan.dev/dragons>" +
                "<https://aksan.dev/writing>"
            )
    ...
    async def today(self, ctx):
        await ctx.send(str(TeryteDate.fromtoday()))

    @group_root.command(
        name="convert",
        aliases=["translate", "equivalent"],
    )
    async def convert(self, ctx, date: str):
        result = False
        try:
            result = TeryteDate.fromdatetime(
                datetime.date.fromisoformat(date)
            )
        except ValueError:
            try:
                result = TeryteDate.fromcommonformat(date).todatetime()
            except AttributeError:
                await ctx.send(
                    "I'm sorry," +
                    "I couldn't parse that, " +
                    "try ISO standard formatting `YYYY-MM-DD` " +
                    "or Federation common format `A.Y.M.D`.")
                return
